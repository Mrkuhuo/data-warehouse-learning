...
1《实时/离线数仓实战》代码，是以电商系统为底座，以电商各类指标统计需求为导向， 开发基于Doris和Piamon、Hudi、Iceberg的离线数仓和实时数仓（数据湖）。离线和实时场景其数据处理逻辑是一致的，只是实现技术不一样，旨在给各位朋友提供一种数仓建设的不同思路参考。代码中有两个版本， **warehouseV1** 和 **warehouseV2** 的区别是V1业务逻辑较为简单，V2业务逻辑比较齐全，V1的模拟数据生产是通过 **Python** 脚本模拟生成，V2的模拟数据是通过 **JAVA** 代码来生成。

...
SELECT
  ...
  cart_id, -- 保留原始值，无需清洗
  product_amount, -- 直接保留商品数量字段的原始值，后续可能根据业务需求验证或转换
  TRY_CAST(price AS FLOAT) AS price, -- 转换价格为浮点数（假设price是以字符串形式存储的
  add_time, -- 确保add_time是合法的TIMESTAMP格式，如有必要，进行转换
  event_time, -- 确保event_time是合法的TIMESTAMP格式，如有必要，进行转换

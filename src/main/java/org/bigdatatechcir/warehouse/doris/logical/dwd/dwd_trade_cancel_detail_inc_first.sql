/*
 * 文件名: dwd_trade_cancel_detail_inc_first.sql
 * 功能描述: 交易域取消订单明细事务事实表(首次加载) - 记录用户取消订单的明细信息
 * 数据粒度: 订单明细取消事件
 ...
 * 1. 首次加载特点:
 *    - 此脚本用于DWD层初始化时一次性执行
 *    - 不使用分区日期(k1)过滤，加载所有历史订单取消数据
 *    - 后续日常加载应使用dwd_trade_cancel_detail_inc.sql或dwd_trade_cancel_detail_inc_per_day.sql
 *    
 * 2. 多表关联设计:
 ...
 *    - 销售预测调整：根据历史取消订单情况调整销售预测模型
 *    - 历史趋势分析：分析不同时期的订单取消率变化
 */
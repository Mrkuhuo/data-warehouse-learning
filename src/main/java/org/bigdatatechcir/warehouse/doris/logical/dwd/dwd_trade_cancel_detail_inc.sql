/*
 * 文件名: dwd_trade_cancel_detail_inc.sql
...
from
    (
        ...
        from ods.ods_order_detail_full                 -- 订单明细表
    ...
    (
        ...
        from ods.ods_order_info_full                   -- 订单主表
 ...
 *    - 地域分析：分析不同省份的订单取消率差异
 *    - 支付转换率优化：减少下单到支付过程中的订单流失
 *
 * 6. 与其他交易事实表的关系:
 *    - 与dwd_trade_order_detail_inc表对比，可分析订单从创建到取消的转化率
 *    - 与dwd_trade_pay_detail_suc_inc表一起，构成完整的订单状态流转视图
 *    - 可结合dwd_trade_cart_full表分析从加购到下单再到取消的全流程
 */
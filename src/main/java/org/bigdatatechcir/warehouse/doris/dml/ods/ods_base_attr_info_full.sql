-- DROP TABLE IF EXISTS ods.ods_base_attr_info_full;
CREATE TABLE ods.ods_base_attr_info_full
(
    `id`           VARCHAR(255) COMMENT '编号',
    `attr_name`    STRING COMMENT '属性名称',
    `category_id`  STRING COMMENT '分类id',
    `category_level` STRING COMMENT '分类层级'
)
...
(
    ...
    "bloom_filter_columns" = "id,attr_name,category_id",  -- 为高频过滤字段创建布隆过滤器，加速WHERE查询

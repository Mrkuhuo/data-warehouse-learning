...
CREATE TABLE dwd.dwd_trade_cart_add_inc
(
    ...
    `create_time`  STRING COMMENT '添加购物车时间',
    `source_id`    STRING COMMENT '来源类型ID',
    `source_type`  STRING COMMENT '来源类型',
    `source_type_name` STRING COMMENT '来源类型名称',
    `sku_num`      BIGINT COMMENT '加购数量',
    `cart_price`   DECIMAL(16, 2) COMMENT '添加购物车时的商品价格'
)
...
(
    ...
    "bloom_filter_columns" = "id,user_id,sku_id",  -- 为高频过滤字段创建布隆过滤器，加速WHERE查询

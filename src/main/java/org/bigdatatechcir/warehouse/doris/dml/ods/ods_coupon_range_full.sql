-- DROP TABLE IF EXISTS ods.ods_coupon_range_full;
CREATE TABLE ods.ods_coupon_range_full
(
    `id`           VARCHAR(255) COMMENT '编号',
    `coupon_id`    STRING COMMENT '优惠券id',
    `range_type`   STRING COMMENT '范围类型 1、商品 2、品类 3、品牌',
    `range_id`     STRING COMMENT '范围id'
)
...
(
    ...
    "bloom_filter_columns" = "id,coupon_id,range_id",  -- 为高频过滤字段创建布隆过滤器，加速WHERE查询

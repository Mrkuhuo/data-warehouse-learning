-- DROP TABLE IF EXISTS ods.ods_sku_image_full;
CREATE TABLE ods.ods_sku_image_full
(
    `id`           VARCHAR(255) COMMENT '编号',
    `sku_id`       STRING COMMENT '商品id',
    `img_name`     STRING COMMENT '图片名称',
    `img_url`      STRING COMMENT '图片路径',
    `spu_img_id`   BIGINT COMMENT '创建时间',
...
(
    ...
    "bloom_filter_columns" = "id,sku_id",  -- 为高频过滤字段创建布隆过滤器，加速WHERE查询

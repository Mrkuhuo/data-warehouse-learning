-- DROP TABLE IF EXISTS dwd.dwd_traffic_action_inc;
CREATE TABLE dwd.dwd_traffic_action_inc
(
    ...
    `version_code`      STRING COMMENT '应用版本号',
    `during_time`       BIGINT COMMENT '持续时间毫秒',
    `page_id`           STRING COMMENT '页面ID',
    `last_page_id`      STRING COMMENT '上页ID',
    `page_item`         STRING COMMENT '页面项目',
    `page_item_type`    STRING COMMENT '页面项目类型',
    `source_type`       STRING COMMENT '来源类型',
    `action_id`         STRING COMMENT '动作ID',
    `action_item`       STRING COMMENT '动作项目',
    `action_item_type`  STRING COMMENT '动作项目类型',
...
(
    ...
    "bloom_filter_columns" = "id,mid_id,user_id,action_id",  -- 为高频过滤字段创建布隆过滤器，加速WHERE查询
